...
const SolicitarLiberacao = () => {
    ...
    const resetForm = () => {
        ...
        setBuscaPlaca('' );
        setDadosVeiculo({ placa: '', marca: '', modelo: '', cor: '', renavan: '', unidade: '', observacoes: '', motivo: '' });
        ultimaPlacaBuscada.current = ''; // também reseta a ref
    };

    //Verificando a loja ativa no momento
    ...
    const handleSubmit = async (e) => {
        ...
        if (veiculo?.unidade !== lojaAtiva.nome) {
            alert(`Este veículo nao pertence ao estoque da "${lojaAtiva.nome}". Operação não permitida.`);
            return;
        }
        ...
        let dados = {
            ...
            unidade: lojaAtiva.nome, data_cadastro: dadosVeiculo.data_cadastro, solicitante, observacoes, motivo: selectedMotivo,
        ...
        const confirmar = window.confirm("Confirmar a solicitação de liberação do veículo? \nEm caso de liberção por devolução, transferência ou venda, a baixa automática do veículo será efetuada às 23:45 de hoje.");
        if (confirmar === true) {
            try {
            ...
            <div className="container d-flex justify-content-center card-container">
                <Box onSubmit={handleSubmit}>
                    ...
                    <Form >
                        ...
                        <div className="col-12 col-md-3">
                            <Input label={"Renavan:"} type={"text"} style={{ width: '150px' }} nameInput={"renavam"} value={dadosVeiculo.renavan} readOnly />
                        </div>
                        {mostrarSelect && (
                            <>
